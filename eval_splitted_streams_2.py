...
from src.Splitter.StreamMapSplitter import StreamMapSplitter
...
def tag_assignment_fct(value: any):
    tuple_id = int(value["myId"])
    if 0 <= tuple_id <= 5:
        yield "id: 0-5"
    if 2 <= tuple_id <= 7:
        yield "id: 2-7"
...
stream_map_splitter = StreamMapSplitter(tag_assignment_fct, tag_assignment_to_stream_id, ds)
splitted_stream = SplittedStream.split_stream(stream_map_splitter, ds, 'myId')
